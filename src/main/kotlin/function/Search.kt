...
private fun buildUrl(query: String): String {
    val url = when (query.startsWith("#")) {
        true -> "https://twitter.com/i/functions.search/timeline?f=tweets&vertical=default&q=$query&src=tyah&reset_error_state=false&"
...
private fun downloadTweets(query: String, maxPosition: String? = null): Pair<String, String> {
    ...
    val headers = mapOf(
        ...
        "model.User-Agent" to "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_6) AppleWebKit/603.3.8 (KHTML, like Gecko) Version/10.1.2 Safari/603.3.8",
        "X-Twitter-Active-model.User" to "yes",
...
private fun parseTweet(tweet: Element): Tweet? {
